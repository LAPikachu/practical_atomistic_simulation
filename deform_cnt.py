...
def run_simulation(loop_number, step_number, timestep, eps):
    for i in range(loop_number):
            ...
            data['temperatur'].append(cnt.get_temperature()/len(cnt))
            data['epot'].append(cnt.get_potential_energy()/len(cnt))
            data['ekin'].append(cnt.get_kinetic_energy()/len(cnt))
            data['etot'].append(cnt.get_total_energy()/len(cnt))
            print(f'Rep {i} of {loop_number}')
            if i%10 == 0:
...
if __name__ == '__main__':
    ...
    #eps = 0.001
    total_strain = 1+eps*loop_number 
    ...
    fig, ax = plt.subplots()
    ax.set_title('strain rate {a:e} $s^-1$, total strain {b} '.format(a=strain_rate, b=total_strain))
